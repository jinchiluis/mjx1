...
def very_smooth_move(robot_api, start_pos, target_pos, step_size=5, min_steps=1, max_steps=100, delay=0.00001):
    """
    ...
    if distance < 150:
        ...
        step_size = 5  # Medium moves get bigger steps (faster)
    else:
    ...
    for i in range(steps + 1):
        ...
        robot_api.move_to(current_x, current_y, current_z, target_pos[3], speed=0, wait_time=0)
        #print(f"Smooth Bewege zu Position: x={current_x:.1f}, y={current_y:.1f}, z={current_z:.1f}")
        time.sleep(delay)

def draw_circle(robot_api, center, radius, steps=50):
